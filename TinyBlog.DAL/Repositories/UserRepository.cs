    ...
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<User> CreateUser(ApplicationContext context, User user)
        {
            using (SqlCommand command = this.helper.GetCommand(@"Insert into [dbo].[User]
                                                        (CreatedBy,UpdatedBy,DateOfJoining,LastActive,Phone,Website,About,ImageUrl,UserId,UserGuid,Name,DateOfBirth,Email,Password,IsActive,Vote,BlogCount,PostCount,CreatedAt,UpdatedAt) 
                                                  values(@CreatedBy,@UpdatedBy,@DateOfJoining,@LastActive,@Phone,@Website,@About,@ImageUrl,@UserId,@UserGuid,@Name,@DateOfBirth,@Email,@Password,@IsActive,@Vote,@BlogCount,@PostCount,@CreatedAt,@UpdatedAt)"))
            {
        ...
        {
            User user = new User();
            using (SqlCommand command = this.helper.GetCommand("select * from [dbo].[User] where UserGuid = '" + UserId + "'"))
            {
                using (DbDataReader reader = await this.helper.ExecuteReaderAsync(command).ConfigureAwait(false))
                {
                    if (reader.HasRows)
        ...
        public async Task<IEnumerable<User>> GetUsers(ApplicationContext context)
        {
            ...
            using (SqlCommand command = this.helper.GetCommand("select * from [dbo].[User]"))
            {
                using (DbDataReader reader = await this.helper.ExecuteReaderAsync(command).ConfigureAwait(false))
                {
                    if (reader.HasRows)
